...
public class RecoverBean implements Serializable{
	...
	public void sendRecoverEmail() throws AddressException, MessagingException {
		if(existEmail()) {
			final String email = "gestaoprocessostesteif@gmail.com";
			final String password = "kers qhmz pflm lzit";
			...
			facesContext.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Esse email não possui cadastro", null));
		}
	}
	...
	public String recoverAccount() {
		System.out.println("codigo random " + randomcode + " codigo recuperacao " + recovercode);
		...
		if(randomcode != null && randomcode.equals(recovercode)) {
			...
			facesContext.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Codigo de Recuperação invalido", null));
			return "";
		}
	}
